...
function findEmployeeBySpecialisation(specialisation) {
    const foundEmployee = employees.find(employee => employee.specialisation === specialisation);
    if (foundEmployee) {
        document.getElementById('employeesDetails').innerHTML = `<p>${foundEmployee.id}: ${foundEmployee.name}: ${foundEmployee.name} - ${foundEmployee.department} - ${foundEmployee.specialisation}</p>`;
    }
    else {
